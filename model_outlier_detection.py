...
def get_model_outliers(model, unlabeled_data, number=20, limit=10000):
    """Get outliers from unlabeled data in training data

    Returns number outliers                                                                                

    An outlier is defined as 
    unlabeled_data with the lowest average logits from inference
    ...
    with torch.no_grad():
        for item in unlabeled_data:
            ...
            layers = model(feature_vector, return_all_layers=True)            
            logits = layers[1]
            ...
            average = logits.sum() / logits.nelement()
            
            item[3] = "logit_outlier"
            item[4] = average
...
if evaluation_count <  minimum_evaluation_items:
    ...
    outliers = get_model_outliers(model, data, number=10)
